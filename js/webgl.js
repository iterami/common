      ...
      'shaders': [
        webgl_shader_create({
          'source':
            ...
            + 'varying lowp vec2 vec_textureCoord;'
            + 'varying lowp vec4 vec_fragmentColor;'
            + 'void main(void){'
            +     'gl_FragColor = vec_fragmentColor * texture2D(sampler, vec_textureCoord);'
            + '}',
        ...
        webgl_shader_create({
          'source':
            ...
            +     'float float_distance = length(gl_Position.xyz);'
            +     'gl_PointSize = 500. / float_distance;'
            +     'vec3 vec_lighting = vec_ambientColor;'
            ...
            +     'if(fog){'
            +         'vec_fragmentColor = mix('
            +           'vec4(vec_clearColor, 1.0),'
            +           'vec_vertexColor,'
            +           'clamp(exp(float_fogDensity * float_distance * -float_distance), 0.0, 1.0)'
            +         ');'
            +     '}'
            +     'vec_fragmentColor = vec_fragmentColor * vec4(vec_lighting, 1.0) * alpha;'
            + '}',
